...
library EnsHelper {
    ...
    function resolverAddressByNamehash(
        bytes32 namehash
    ) public view returns (address resolverAddress) {
        resolverAddress = IEnsRegistry(registryAddress).resolver(namehash);
    }

    function resolverAddressByName(
        string memory name
    ) public view returns (address resolverAddress) {
        bytes32 namehash = namehashByName(name);
    ...
    function namehashByName(
        ...
        for (uint256 partIdx = parts.length - 1; partIdx >= 0; partIdx--) {
            ...
            namehash = keccak256(
                abi.encodePacked(
                    namehash,
                    keccak256(abi.encodePacked(bytes(part)))
                )
            );
            if (partIdx == 0) {
                break;
