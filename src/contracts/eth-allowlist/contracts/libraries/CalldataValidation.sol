...
library CalldataValidation {
     ...
     */
    function methodSignatureByCondition(
        IAllowlist.Condition memory condition
    ) public pure returns (string memory signature) {
        bytes memory signatureBytes = abi.encodePacked(
            condition.methodName,
            "("
        );
        for (
            uint256 paramIdx;
            paramIdx < condition.paramTypes.length;
            paramIdx++
        ) {
    ...
    function checkMethodSelector(
        ...
        methodSelectorValid =
            methodSelectorBySignature == methodSelectorByCalldata;
    }
   ...
   */
    ...
    function validateCalldataByAllowlist(
        ...
        for (
            ...
            bool conditionPassed = verifyCondition(
                allowlistAddress,
                condition,
                targetAddress,
                data
